...
sequenceDiagram
    ...
    alt 서명 성공
        ...
        pg사->>앱카드 서버:48. 최종 결제 상태 전달
    else 서명 실패
        앱카드 서버->>앱카드: 31. 인증 실패 응답
        앱카드->>사용자: 32. 인증 실패 화면 표시
        앱카드 서버->>pg사: 33. 인증 실패 알림
        pg사->>원큐 오더 서버: 34. 실패 응답
        pg사->>앱카드 서버:35. 최종 결제 상태 전달
        원큐 오더 서버->>원큐 오더 서버: 36. 주문 상태 AUTH_FAILED
        원큐 오더 서버->>원큐 오더 웹: 37. 실패 응답
        원큐 오더 웹->>사용자: 38. 결제 실패 화면 표시
    end
